...
function EvidenceTrace({ evidence }: { evidence: LaborBriefingReady['readingEvidence'][number] }) {
  ...
  return (
    <section className="briefing-trace__indicator">
      ...
      <dl>
        ...
        </> : direction ? <div><dt>Direction</dt><dd>{direction.evidence.replaceAll('-', ' ')}</dd></div> : null}
    ...
    <article className="labor-briefing" aria-labelledby="labor-briefing-question">
      ...
      <details className="briefing-disclosure">
        ...
        <div className="briefing-disclosure__content">
          {model.readingEvidence.map((evidence) => <EvidenceTrace key={evidence.id} evidence={evidence} />)}
          <section><h4>Dimension result</h4><dl>
            <div><dt>Condition inputs</dt><dd>Unemployment anchor and prime-age employment confirmer</dd></div>
            <div><dt>Condition reason</dt><dd>{model.conditionReading.reason}; final {model.conditionLabel}</dd></div>
            <div><dt>Direction inputs</dt><dd>Payroll movement anchor and unemployment movement confirmer</dd></div>
            ...
            <div><dt>Visual context</dt><dd>10-year unemployment range {model.sparkline.minimum.toFixed(1)}%–{model.sparkline.maximum.toFixed(1)}%; latest {model.sparkline.latest.value.toFixed(1)}%; comparison median {model.sparkline.median.toFixed(1)}%; interquartile band {model.sparkline.lowerQuartile.toFixed(1)}%–{model.sparkline.upperQuartile.toFixed(1)}%. The band is not a confidence interval.</dd></div>
          </dl></section>
        </div>
      </details>
      <details className="briefing-disclosure">
