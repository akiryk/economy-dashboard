...
import { buildLatestYearSummary, defaultSavingRateComparison, describeDistributionObservation, formatDistributionRate, latestValidDistributionYear } from './savingRateDistribution'
...
describe('saving-rate distribution helpers', () => {
  it('selects the latest valid year and required defaults', () => {
    expect(latestValidDistributionYear(data)).toBe(
      Math.max(...data.observations.filter(({ rate }) => rate !== null).map(({ year }) => year)),
    )
  ...
  it('summarizes every latest-year decile and the valid coverage', () => {
    const years = data.observations.filter(({ rate }) => rate !== null).map(({ year }) => year)
    const latestYear = Math.max(...years)
    const latest = data.observations.filter(({ year }) => year === latestYear)
    const summary = buildLatestYearSummary(data)
    expect(summary).toContain(`${Math.min(...years)} through ${latestYear}`)
    for (const observation of latest) {
      expect(summary).toContain(formatDistributionRate(observation.rate))
    }
