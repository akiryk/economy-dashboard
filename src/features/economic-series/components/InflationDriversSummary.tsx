...
export function InflationDriversSummary({
  ...
  const summary =
    model?.summary ?? "Inflation contribution data are unavailable.";
  ...
  const maxMagnitude = Math.max(
    0.01,
    ...(model?.displayedContributions ?? []).map(({ contribution }) =>
    ...
    >
      <div className="inflation-contributions__comparison">
        <section className="inflation-contributions__section">
          <h4>Contribution to inflation over the past 12 months</h4>
            ...
            {model?.displayedContributions.map((category) => {
              ...
              return (
                <div className="inflation-contributions__row" key={category.id}>
                  <span className="inflation-contributions__label">
                    {category.label}
                  </span>
        ...
        <section className="inflation-contributions__section">
          <div className="inflation-contributions__section-heading">
            ...
            >
              <p>
                Each bar shows how many percentage points a category contributed
                to the current 12-month headline CPI inflation rate. The lines
                show five years of each mapped category’s own year-over-year
                inflation rate in percent.
              </p>
              <p>
                The four categories with the largest absolute current
                contributions are selected dynamically. Everything else is the
                net remainder. Right-side mappings use exact category IDs;
                unsupported categories are omitted without replacement, which
                does not mean their prices had no inflation.
              </p>
              <p>
                Contribution depends on both price movement and CPI weight, so a
              ...
              <p>
                ...
                can focus a chart and use the left and right arrow keys to move
                among published observations; missing months are skipped rather
                than interpolated.
              </p>
            </CompactChartHelp>
          </div>
          <p className="inflation-category-trends__note">
            Shown for current contributors with a directly comparable CPI
            series.
          </p>
          <Suspense
            fallback={
              <p
                className="inflation-category-trends__unavailable"
                role="status"
              >
      ...
      <figcaption
        ...
        id="inflation-contributions-summary"
      >
  ...
  return (
    <CompactMetricCardLayout
      ...
      measureLabel={
        <span className="inflation-drivers-measure">
          <span>Category contributions to overall CPI inflation</span>
          <CompactChartHelp
            buttonLabel="Explain overall CPI inflation"
            dialogLabel="Overall CPI inflation explanation"
          >
            <p>
              <strong>Overall, or “headline,” CPI inflation</strong> includes
              all consumer-price categories, including food and energy.
              Economists often contrast it with <strong>core CPI</strong>, which
        ...
        <>
          ...
          <div className="series-explanations">
            ...
            <section>
              ...
              <p>
                ...
                weights; published effects are rounded to three decimals and can
                be revised with CPI source data. No missing value is
          ...
          <footer className="series-supporting">
            <p className="series-source">
              Source:{" "}
              <a
                href={contributionData.sourceUrl}
                target="_blank"
                rel="noreferrer"
              >
