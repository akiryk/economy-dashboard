...
export function ProductivityLevelSummary({ series }: { series: EconomicSeries }) {
  ...
  const summary = calculateChartSummary(normalized)
  ...
  return <article id="labor-productivity-level-card" className="series-card" aria-labelledby="labor-productivity-level-question">
    ...
    <TimeRangeControl selectedRange={selectedRange} onRangeChange={setSelectedRange} contextLabel="Productivity over time" />
    <Suspense fallback={<p className="chart-state">Loading chart visualization…</p>}><EconomicTimeSeriesChart kind="single" observations={normalized} seriesName="Productivity index, selected-range baseline = 100" frequency="quarterly" units="Index" transformation={series.transformation} includeZero={false} valueFormat="index" /></Suspense>
    <p className="chart-summary" aria-live="polite">From {baseline ? formatObservationPeriod(baseline.date, 'quarterly') : 'an unavailable baseline'} to {latest ? formatObservationPeriod(latest.date, 'quarterly') : 'an unavailable latest quarter'}, productivity changed {formatSignedPercentage(latest?.changeFromBaseline ?? null)}. The normalized index ranged from {summary.minimum?.value?.toFixed(1) ?? 'unavailable'} in {summary.minimum ? formatObservationPeriod(summary.minimum.date, 'quarterly') : 'an unavailable quarter'} to {summary.maximum?.value?.toFixed(1) ?? 'unavailable'} in {summary.maximum ? formatObservationPeriod(summary.maximum.date, 'quarterly') : 'an unavailable quarter'}, across {summary.observationCount} valid observations.</p>
    ...
    <footer className="series-supporting"><p className="series-source">Source: <a href={series.sourceUrl} rel="noreferrer" target="_blank">{series.sourceName}</a></p><details className="supporting-disclosure"><summary>Series details</summary><dl className="series-metadata"><div><dt>Provider series identifier</dt><dd>{series.providerSeriesId}</dd></div><div><dt>Frequency</dt><dd>Quarterly</dd></div><div><dt>Transformation</dt><dd>{series.transformation}</dd></div><div><dt>Retrieved</dt><dd>{formatDate(series.retrievedAt)}</dd></div></dl></details><details className="supporting-disclosure"><summary>Recent observations</summary><ProductivityLevelTable observations={normalized} /></details></footer>
  </article>
}
