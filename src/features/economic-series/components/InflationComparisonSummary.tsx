...
export function InflationComparisonSummary({
  ...
  const id = momentum ? 'recent-inflation-momentum' : 'headline-versus-core-inflation'
  const title = momentum ? 'Recent Inflation Momentum' : 'Headline Versus Core CPI'
    ...
    <article className="series-card" aria-labelledby={`${id}-question`}>
      ...
      <div className="series-current" aria-label={latestLabel}>
        ...
        <p className="series-current__comparison">
          Corresponding headline rate: {formatSignedPercentage(latest?.headline ?? null)}
        </p>
      </div>
      ...
      {summary.observationCount > 0 ? (
        <>
          ...
          <p className="chart-summary" aria-live="polite">
            In {latest ? formatObservationPeriod(latest.date, 'monthly') : 'an unavailable month'}, the headline rate was{' '}
            {formatPercentage(latest?.headline ?? null)} and the core rate was{' '}
            {formatPercentage(latest?.core ?? null)}.
            {!momentum && (
              <> Core was {latest?.difference !== null && latest?.difference !== undefined && latest.difference >= 0 ? 'above' : 'below'} headline by{' '}
                {formatSignedPercentage(latest?.difference ?? null)} percentage points.</>
            )}{' '}
            Core ranged from {formatPercentage(summary.minimum?.value ?? null)} in{' '}
            {summary.minimum ? formatObservationPeriod(summary.minimum.date, 'monthly') : 'an unavailable month'} to{' '}
            {formatPercentage(summary.maximum?.value ?? null)} in{' '}
            {summary.maximum ? formatObservationPeriod(summary.maximum.date, 'monthly') : 'an unavailable month'}.
            {momentum && (
              <> The latest core rate was {formatPercentage(latest?.core ?? null)}, compared with {formatPercentage(priorCore)} three months earlier.</>
            )}
          </p>
        </>
      ) : (
        <p className="chart-state">No aligned inflation observations are available.</p>
      )}
      ...
      <div className="series-explanations">
        <section>
          ...
          <p>{momentum
            ...
            : 'Headline CPI includes the full consumer basket. Core CPI excludes food and energy, which can be especially volatile. Comparing them helps show whether recent inflation is concentrated in those categories or is also present across the rest of the basket.'}</p>
        </section>
        <section>
          ...
          <p>{momentum
            ...
            : 'Core CPI still includes many categories that can move unevenly, and excluding food and energy does not make those costs irrelevant to households. CPI is a national average and may not match an individual household’s expenses.'}</p>
        </section>
      </div>
      ...
      <footer className="series-supporting">
        <p className="series-source">
          Sources:{' '}
          <a href={headline.sourceUrl} rel="noreferrer" target="_blank">Headline CPI via FRED</a>
          {'; '}
          <a href={core.sourceUrl} rel="noreferrer" target="_blank">Core CPI via FRED</a>
        </p>
        <details className="supporting-disclosure">
          ...
          <dl className="series-metadata">
            ...
            <div><dt>Transformation</dt><dd>{core.transformation}</dd></div>
            <div><dt>Retrieved</dt><dd>{formatDate(core.retrievedAt)}</dd></div>
            <div><dt>Observation coverage</dt><dd>{coverageStart && coverageEnd ? `${formatObservationPeriod(coverageStart.date, 'monthly')} to ${formatObservationPeriod(coverageEnd.date, 'monthly')}` : 'Not available'}</dd></div>
          </dl>
        </details>
        <details className="supporting-disclosure">
