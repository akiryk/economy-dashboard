...
export function ManufacturingComparisonSummary({ output, employment }: { output: EconomicSeries; employment: EconomicSeries }) {
  ...
  let normalized
  try {
    normalized = normalizeManufacturingComparison(selected)
  } catch {
    normalized = null
  }
  ...
  return <article id="manufacturing-output-versus-employment-card" className="series-card" aria-labelledby="manufacturing-output-versus-employment-question">
    ...
    <TimeRangeControl selectedRange={selectedRange} onRangeChange={setSelectedRange} contextLabel="Manufacturing output versus employment" />
    {normalized && baseline && latest ? <>
      <Suspense fallback={<p className="chart-state">Loading chart visualization…</p>}><EconomicTimeSeriesChart kind="manufacturing-comparison" outputObservations={toNormalizedObservations(normalized, 'normalizedOutput')} employmentObservations={toNormalizedObservations(normalized, 'normalizedEmployment')} frequency="monthly" /></Suspense>
      <p className="chart-summary" aria-live="polite">From the selected-range baseline in {formatObservationPeriod(baseline.date, 'monthly')} through {formatObservationPeriod(latest.date, 'monthly')}, manufacturing output changed {formatSignedPercentage(outputChange)} and manufacturing employment changed {formatSignedPercentage(employmentChange)}. The two measures moved in {direction} over the complete interval. Changing the range changes the baseline.</p>
