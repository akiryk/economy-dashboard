...
describe('DashboardPage economic series', () => {
  ...
  it('renders annual budget balance and quarterly debt held by the public after Financial conditions', async () => {
    ...
    expect(debtCompactProps.model.recentObservations).toHaveLength(21)
    expect(debtCompactProps.definition.showZeroLine).toBe(false)
    expect(debtCompactProps.definition.showLatestMarker).toBe(true)
    expect(debtCompactProps.definition.interactiveDetails).toBe(true)
    expect(within(debt).queryByText(/higher interest rates become more costly/)).not.toBeInTheDocument()
    const debtContextToggle = within(debt).getByRole('button', { name: 'Why this matters for federal debt' })
    await user.click(debtContextToggle)
    expect(within(debt).getByText(/higher interest rates become more costly/)).toBeVisible()
    expect(within(debt).getByRole('button', { name: 'Hide context for federal debt' })).toBeVisible()
    expect(within(debt).queryByRole('button', { name: 'Maximum' })).not.toBeInTheDocument()
    await user.click(within(budget).getByRole('button', { name: /More/ }))
    expect(within(budget).getByText('This measure shows whether federal spending exceeded revenue or vice versa, relative to the size of the economy.')).toBeVisible()
    expect(within(budget).getByText(/Persistent deficits add to federal debt/)).toBeVisible()
    expect(within(budget).getByText(/does not show why the deficit changed/)).toBeVisible()
    await user.click(within(debt).getByRole('button', { name: /More/ }))
    expect(within(debt).getByText('This measure shows the accumulated federal debt held outside federal government accounts relative to the size of the economy.')).toBeVisible()
    expect(within(debt).getByText(/^The ratio does not identify a precise crisis threshold/)).toBeVisible()
