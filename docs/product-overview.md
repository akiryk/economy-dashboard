...
The separate `/dashboard` route is a simplified status board. Its nine implemented tiles cover real GDP growth with nominal GDP context; standalone unemployment; payroll growth using the latest complete three-month average with the latest single-month change as secondary context; initial claims using the official four-week average and latest weekly reading; headline CPI with core CPI context; the market-implied 10-year breakeven inflation rate; the effective federal funds rate with the target-range upper bound; the 10-year-minus-2-year yield spread with 10-year-minus-3-month context; and the real-time Sahm Rule indicator. The prices-and-rates row places realized inflation, expected inflation, the policy rate, and the yield curve together. Each tile front remains a glanceable status view and flips to a same-size contextual back whose concise explanation responds deterministically to current data and metric-specific rules; no runtime AI commentary is used. The interaction supports pointer, touch, keyboard, and reduced-motion users. It does not replace the research-oriented home page, and the remaining three markets-and-credit tiles are not yet shipped.
