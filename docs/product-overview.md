...
Its page header is deliberately minimal: `U.S. Economy` followed by the dashboard date. When an automatic refresh fails before new data can be validated and deployed, a bold red `Data is possibly out of date.` alert appears immediately below that date. Section headers contain only their titles; explanatory context belongs inside the relevant cards rather than in introductory paragraphs.
