...
- The right side shows five-year year-over-year category inflation rates only for current top-four contributors with an explicit directly comparable CPI-U mapping. Shelter, Energy, and Food are initially mapped; derived Other services is omitted without substitution. Rate lines share one padded scale containing zero, retain negative values and null gaps, and display `%` rather than the left side’s `pp`. Wide layouts place current contributions beside the available rate histories; narrow layouts stack them. One nonvisual summary states current contributions, mapped rates and periods, omissions, units, window, and scale behavior. The expanded research content is unchanged.
