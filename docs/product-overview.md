...
| Are workers’ wages keeping up with prices? | Monthly real wage growth. The compact five-year line labels its actual start and end months and uses zero to separate wages outpacing prices from prices outpacing wages; values within 0.1 percentage point of zero are described as about even. More retains nominal wage growth, headline CPI inflation, derived real wage growth, and the full research controls. | Connects inflation to aggregate purchasing power for a defined group of private-sector workers without implying that the average describes every worker. |
