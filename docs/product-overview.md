...
Its page header is deliberately minimal: `U.S. Economy` followed by the date of the most recent successful substantive data ingestion. That date advances only when changed provider data pass validation and are accepted for deployment; code-only deployments, unchanged refresh checks, and failed refreshes preserve the prior date. Missing or malformed refresh metadata produces the neutral heading `U.S. Economy`. When an automatic refresh fails before new data can be validated and deployed, a bold red `Data is possibly out of date.` alert appears immediately below the preserved date. Section headers contain only their titles; explanatory context belongs inside the relevant cards rather than in introductory paragraphs.
