...
The home page (`/`) contains **27 cards in nine sections**: Growth; Prices; Employment and income; Households; Housing; Business and manufacturing; Financial conditions; Government finances; and Trade and tariffs. The measures update at different frequencies and may point in different directions. No single card is an overall verdict on the economy.

Its page header is deliberately minimal: `U.S. Economy` followed by the date of the most recent successful substantive data ingestion. That date advances when any changed provider data pass complete snapshot verification and are accepted for deployment, including during a partial-success refresh; code-only deployments and unchanged refresh checks preserve the prior date. Missing or malformed refresh metadata produces the neutral heading `U.S. Economy`. When a global refresh, verification, or deployment failure prevents a trustworthy snapshot from reaching production, a bold red `Data is possibly out of date.` alert appears immediately below the preserved date. An isolated refresh-unit failure instead appears only on cards or tiles that use its affected datasets. Section headers contain only their titles; explanatory context belongs inside the relevant cards rather than in introductory paragraphs.

The application also has a top-level international comparison page at `/compare`, an unlinked secondary-indicators page at `/secondary`, and a simplified status board at `/dashboard`. They are supporting presentations, not part of the home-page inventory below. The status board contains ten single-purpose tiles: year-over-year real GDP growth using the same `GDPC1` dataset as the home-page GDP card; standalone unemployment; payroll growth using the latest complete three-month average with the latest single-month change as secondary context; initial claims using the official four-week average with the latest weekly reading; headline 12-month CPI-U inflation; real wage growth for all private employees; the real-time Sahm Rule indicator; the Freddie Mac 30-year fixed mortgage rate; the S&P 500 prior closing index level; and the high-yield corporate credit spread. The S&P 500 tile is descriptive prior-close economic context, not an intraday or real-time quote, and always retains its explicit observation date. GDP readings within 0.1 percentage point of zero are labeled **Little changed**; larger positive readings are **Growing**, and larger negative readings are **Contracting**, without treating the GDP measure alone as a recession call. Each tile's hero and sparkline show the same measure. Tile fronts remain glanceable status views and flip to same-size contextual backs whose concise explanations respond deterministically to current data and metric-specific rules; no runtime AI commentary is used. The interaction supports pointer, touch, keyboard, and reduced-motion users. The status board does not replace the research-oriented home page.

An OECD refresh failure retains the complete last-known-good comparison snapshot and appears as a single scoped notice on `/compare`; it does not trigger the home-page warning or imply that primary U.S. data are stale.

## International comparison page

The `/compare` page gives the U.S. readings context without turning unlike national statistics into a composite score. It compares the United States with Australia, Canada, France, Germany, Italy, Japan, South Korea, Spain, and the United Kingdom across five OECD measures: prime-age employment, unemployment, headline consumer-price inflation, year-over-year real GDP growth, and ten-year government-bond yields. Each module shows the actual period for every country, marks the United States in text as well as color, states whether numeric ordering is directional or merely descriptive, and preserves unavailable observations as `N/A`. Source and methodology disclosures explain the exact OECD series, comparability limits, and freshness rules. Countries can have different latest periods; values are not forward-filled to create false alignment. The page is a compact reference view rather than a substitute for the U.S.-focused research cards.
...
Freshness exceptions supplement this pattern without becoming permanent card chrome. Healthy data show only their normal observation period. A neutral inline notice distinguishes a provider that has not advanced from a manual or indeterminate source; a stronger text-and-border treatment identifies data known to be overdue or blocked by failure. A failed automatic refresh says `Data update failed` and explains that the last successfully validated observation remains visible. The same sanitized dataset state is reused on research cards, status tiles, and comparisons, supports simultaneous failures, and clears automatically after a successful source check even when no newer observation was published. No UI code infers staleness merely from an observation's age.
...
| **How has workers’ purchasing power changed over time?** | Latest rolling 10-year change in real hourly earnings for production and nonsupervisory private-sector employees, neutral more/less/little-change wording, and roughly 40 years of the 10-year measure with a zero line and no percentile bands. | Full 4-, 10-, or 20-year rolling history, keyboard/touch-accessible exact values, start/end AHETPI and CPI-W levels, exact-ratio decomposition, methodology, limitations, sources, and observations. | Complements the preceding short-term all-private card with one consistent long-run worker population. CPI-W follows BLS real-earnings methodology for this group; overlapping rolling periods describe outcomes without assigning causes. |
...
| **How much has the cost of building a comparable home risen?** | Latest monthly Census constant-quality construction-cost index level, its 2005 base, cumulative increase since the base, subordinate annual/monthly changes, and roughly 20 years of index levels. | Full range and zoom controls, definitions and exclusions, monthly observations, and a visibly separate annual history of contractor-built median contract price per square foot in nominal dollars. | Shows the long-run national cost level for comparable-quality construction while providing familiar annual dollar context without splicing the two measures into a synthetic monthly dollar estimate. |
