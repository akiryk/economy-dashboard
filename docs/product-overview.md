...
The separate `/dashboard` route is a simplified status board. Its six implemented tiles cover real GDP growth with nominal GDP context; standalone unemployment; payroll growth using the latest complete three-month average with the latest single-month change as secondary context; initial claims using the official four-week average and latest weekly reading; the real-time Sahm Rule indicator; and headline CPI with core CPI context. The payroll tile compares the derived three-month pace with the full history of the same measure and preserves incomplete or nonconsecutive windows as gaps. Each tile front remains a glanceable status view and flips to a same-size contextual back whose concise explanation responds deterministically to current data and metric-specific rules; no runtime AI commentary is used. The interaction supports pointer, touch, keyboard, and reduced-motion users. It does not replace the research-oriented home page, and the remaining six dashboard tiles are not yet shipped.
