...
The separate `/dashboard` route is a simplified status board. Its five implemented tiles cover real GDP growth with nominal GDP context, unemployment with monthly payroll change, initial claims using the official four-week average and latest weekly reading, the real-time Sahm Rule indicator, and headline CPI with core CPI context. It does not replace the research-oriented home page, and the remaining six dashboard tiles are not yet shipped.
