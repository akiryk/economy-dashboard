...
| Has inflation picked up in recent months? | Direct comparison of overall CPI inflation over the latest 12 months with the latest three-month annualized overall CPI pace. The compact answer uses their percentage-point difference; More retains 12-month and three-month annualized headline-versus-core evidence. | Distinguishes the observed recent pace from the slower-moving past-year rate without treating annualization as a forecast or allowing core CPI to override the headline comparison. |
| Are workers’ wages keeping up with prices? | Monthly real wage growth, labeled as year-over-year wage growth after adjusting for inflation. The compact five-year line labels its actual start and end months and uses zero to separate wages outpacing prices from prices outpacing wages; values within 0.1 percentage point of zero are described as about even, and rounded zero is displayed as `0%`. More retains nominal wage growth, headline CPI inflation, derived real wage growth, and the full research controls. | Connects inflation to aggregate purchasing power for a defined group of private-sector workers without implying that the average describes every worker. |
