...
| 17 | How much has the cost of building a comparable home risen? | Census fixed-weight monthly workbook → `single-family-construction-cost-index.json`; separate annual contractor-built workbook → `contractor-built-price-per-square-foot.json` | monthly Jul 2026; annual 2025 | CENSUS-COST, CENSUS-CHARS |
...
| BLS-EMP | BLS Employment Situation via FRED: `UNRATE`, `LNS12300060`, `PAYEMS`, `CES0500000003`, and production/nonsupervisory earnings `AHETPI` (BLS `CES0500000008`); `SAHMREALTIME` via FRED | Monthly, normally early the following month on the BLS calendar. Payroll/wage estimates receive monthly and annual benchmark revisions. | `data:refresh`; automatic daily | Healthy when the latest scheduled Employment Situation month is deployed. Before release: healthy. After BLS release but before FRED propagation: late/provider not advanced. The long-run purchasing-power foundation is current only through the newest month shared with CPI-W. | Grouped payroll and wage derivations preserve prior files. The purchasing-power group preserves both inputs and all four derived files together. Inspect BLS release, FRED, workflow, then parsing/derivation tests. Never splice worker populations or force a rounded result. |
| BLS-CPI | BLS CPI release via FRED (`CPIAUCNS`, `CPIAUCSL`, `CPILFESL`, and CPI-W `CWSR0000SA0`) and BLS API category indexes (`CUUR0000SAH1`, `CUUR0000SA0E`, `CUUR0000SAF1`) | Monthly on the BLS calendar, generally around mid-month for the prior month; seasonal factors and indexes may be revised under source policy. | `data:refresh`; automatic daily | Healthy through latest scheduled CPI month. The long-run purchasing-power foundation remains at the latest exact month shared by `AHETPI` and `CWSR0000SA0`; a newer single component is provider propagation, not permission to carry it forward. Unexpected after both inputs advanced plus one successful daily cycle. | CPI files replace as rollback-protected groups. Purchasing-power input and derived files form another atomic group. Inspect source IDs, exact months, gaps, and workflow. Parser/validation failures require code repair, never interpolation. |
...
| CENSUS-COST | Census/HUD Survey of Construction `price_uc_cust.xlsx`, Vertical-sheet Laspeyres constant-quality index, 2005=100 | Monthly with New Residential Sales. Recent observations may be preliminary or revised. | `data:refresh` (`data:refresh-home-construction-cost`); automatic daily | Healthy through the latest published workbook month. Unexpected after the workbook advances and one successful daily cycle does not deploy it. | Preserve the last-good monthly file on retrieval/schema failure. Accept revisions, require contiguous source months, and render the published level without smoothing. This unit has no CPI or annual-benchmark dependency. |
| CENSUS-CHARS | Census/HUD Characteristics of New Housing `contractpricesqft_cust.xls`, `ContractMedAvgPriceSqFt` national median | Annual, generally published in the following year; history and recent years may be revised. | `data:refresh` (`data:refresh-contractor-price-per-square-foot`); automatic daily | Healthy through the latest published annual workbook year. Unexpected after the official workbook advances and one successful daily cycle does not deploy it. | Preserve the last-good annual file on retrieval/schema failure. Require contiguous annual history and nominal positive values. Do not carry an observation forward, convert it to real dollars, or splice it to the monthly index. This unit can advance or fail independently of CENSUS-COST. |
...
| Provider unreachable, timeout, HTTP error | Yes in per-unit command logs; normal scheduled units and OECD use bounded retry. Exhausted normal-unit retries preserve affected artifacts while the complete mixed snapshot remains eligible for verification and publication. | Inspect the unit result and preserved artifact before attributing fault. Scoped product state and owner reporting are completed in the next epic story. |
...
| Derived generation/validation/write failure | Yes; atomic/group rollback preserves good files. A classified unit failure remains scoped, while an atomicity or orchestration exception exits nonzero and blocks globally. | The evaluator records a dataset-specific pipeline failure when supplied by automation. |
