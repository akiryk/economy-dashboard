...
Product review on July 20, 2026 found the current 12-quarter path legible at both a 662×192 desktop plot and a 278×192 narrow plot, with no page overflow and a valid responsive canvas after resizing. The recent window begins in 2023 Q2, so pandemic-era extremes do not distort this particular view. The two yellow-neutral bands remain visibly distinct, the blue line remains legible across both, and the separated dashed zero line adds clear contraction context without implying a target. This review uses `markArea`, not stacked area series. The production dashboard contained no compact-chart instance during verification.
