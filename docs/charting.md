...
The real-wage card uses one nonsmoothed five-year line of year-over-year real growth in average hourly earnings for all private employees. It preserves null gaps, marks the latest endpoint, and always shows zero: positive values mean wages gained purchasing power and negative values mean they lost purchasing power. Hover, tap, focus, and arrow-key interaction expose exact monthly values. A pure classifier uses an explicit ±0.1-point neutral band. Historical bands use all compatible derived history beginning in March 2007 rather than implying a full 25-year comparison. The nominal all-private wage and seasonally adjusted CPI-U comparison, exact-ratio methodology, full range and zoom controls, table, sources, and aggregate limitations remain under More. The status-dashboard tile uses the same derived observations for both its hero and five-year sparkline, with zero as its reference line.
...
Story 61 compacts the prime-age employment card around its latest level and a deterministic higher-is-better historical position. Its 61-month line preserves null gaps and appears over trailing 25-year middle-50% and middle-80% bands calculated from finite observations, with a latest marker and no zero line. Exact percentile boundaries map the lower outer edge to low, the inner edges to typical, and the upper outer edge to high. The compact chart labels its visible start and end months; hover, tap, focus, and arrow keys expose exact monthly percentages. Help defines ages 25–54, explains why this range is less affected by retirement and schooling, and states the measure’s limits. More retains the existing full historical chart, controls, summary, interpretation, related indicators, and source information.
...
`EconomicTimeSeriesChart` initializes one ECharts instance for its container, updates options when filtered observations change, and disposes the instance on unmount. Semantically unchanged parent renders are memoized so they do not clear an active tooltip. Compact historical charts likewise keep stable defaults for optional configuration, preventing parent renders from destroying and recreating their ECharts instance. A `ResizeObserver` resizes each chart when its container changes; the global window resize event is a fallback for browsers without `ResizeObserver`.
