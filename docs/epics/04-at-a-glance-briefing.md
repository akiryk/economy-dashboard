...
**Story 2 — one vertical slice: the Labor tile.** A `/briefing` route (not the default) rendering the full tile anatomy for Labor only, template sentences included, using Story 1's engine. Labor goes first because it has the richest stock/flow structure, a live normalization-versus-weakness question in current data, and the timeliest inputs — if the framework works anywhere convincingly, it must work here.

**Step 3 — human review checkpoint against real data.** Judge the labor tile's chips, sentence, sparkline, and trace against the actual research card evidence. Expected outcome is threshold and template tuning in `briefing-rules.md`; possible outcome is framework revision. Nothing scales past this gate.
